...
RUN npm install --omit=dev @elizaos/core @elizaos/plugin-discord winston
...
RUN echo '#!/bin/bash\n\
echo "[STARTUP] $(date): Starting Discord Bot worker service" > /app/logs/startup.log\n\
\n\
...
\n\
# Run the direct discord test script\n\
echo "Starting Discord Bot..." | tee -a /app/logs/startup.log\n\
node --experimental-modules direct-discord-test.js 2>&1 | tee -a /app/logs/bot.log\n\
\n\
